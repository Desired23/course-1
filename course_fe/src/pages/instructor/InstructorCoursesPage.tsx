...
import { getAllCourses, type CourseListItem, formatPrice, parseDecimal, getLevelLabel, formatDuration } from '../../services/course.api'
...
export function InstructorCoursesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const allCourses = await getAllCourses({ instructor_id: profile.id })
  ...
  const filteredCourses = courses.filter(course => {
    ...
    if (selectedTab === "published") return matchesSearch && course.status === "published"
    if (selectedTab === "draft") return matchesSearch && course.status === "draft"
    if (statusFilter !== "all") return matchesSearch && course.status === statusFilter
    
    return matchesSearch
  }).sort((a, b) => {
    switch (sortBy) {
      case 'students': return b.total_students - a.total_students
      case 'revenue': return parseDecimal(b.price) - parseDecimal(a.price)
      case 'rating': return parseDecimal(b.rating) - parseDecimal(a.rating)
  ...
  const totalStats = {
    ...
    avgRating: (() => {
      const rated = courses.filter(c => parseDecimal(c.rating) > 0)
      return rated.length ? rated.reduce((sum, c) => sum + parseDecimal(c.rating), 0) / rated.length : 0
    })(),
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                            ...
                            {course.status === 'published' && (
                              <>
                                ...
                                <div className="text-center">
                                  <p className="font-semibold text-sm md:text-base">{formatPrice(course.price)}</p>
                            ...
                            <div className="flex items-center gap-2 text-xs sm:text-sm">
                              ...
                              {course.discount_price && (
                                <>
                                  ...
                                  <span className="text-green-600">{t('instructor_courses.discount')}: {formatPrice(course.discount_price)}</span>
                                </>
                              )}
                            </div>
                          </div>
