...
export function WishlistPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    getAllWishlistByUser(parseInt(user.id))
      .then((items) => { if (!cancelled) setWishlistItems(items) })
  ...
  const handleAddToCart = async (item: WishlistItem) => {
    if (!user?.id) return
    try {
      await addToCartApi({ user: parseInt(user.id), course: item.course })
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              {wishlistItems.length} khóa học trong wishlist
            </p>
          </div>
              ...
              <Card key={item.id} className="group hover:shadow-lg transition-shadow">
                ...
                <CardHeader className="pb-3">
                  <CardTitle className="line-clamp-2 cursor-pointer hover:text-primary"
                    onClick={() => navigate(`/courses/${item.course}`)}>
                    {course.title}
                  </CardTitle>
